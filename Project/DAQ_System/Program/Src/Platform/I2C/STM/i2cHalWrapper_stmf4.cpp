...
namespace platform {
	...
	void i2cHalWrapperStmf4::Transmit(uint8_t *commands, uint8_t dev_add_, uint8_t maxBytes){
		HAL_I2C_Master_Transmit(&bx_i2c_,dev_add_, commands, maxBytes, HAL_MAX_DELAY);
	}
	void i2cHalWrapperStmf4::Receive(uint8_t *response, uint8_t dev_add_, uint8_t response_size_){
