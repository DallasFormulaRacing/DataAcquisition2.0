...
namespace platform {
	...
	void i2cHalWrapperStmf4::Transmit(uint8_t *commands, uint8_t dev_add_){
		HAL_I2C_Master_Transmit(&bx_i2c_,dev_add_, commands, kMaxBytes, HAL_MAX_DELAY);
	}
	void i2cHalWrapperStmf4::Receive(uint8_t *response, uint8_t addr_reg_, uint8_t dev_add_){
		HAL_I2C_Master_Transmit(&bx_i2c_, dev_add_, &addr_reg, 1, HAL_MAX_DELAY); //send a transmit to the register interested in

		uint8_t response_size_ = sizeof(response)/sizeof(unint8_t); //derive response size from the expected size of the response
